...
SymbolEntry * newFunction (const char * name)
{
    ...
    if (e == NULL) {
        ...
        if (e != NULL) {
            ...
            e->u.eFunction.firstArgument = e->u.eFunction.lastArgument = e->u.eFunction.curr_par = NULL;
...
SymbolEntry * lookupEntry (const char * name, LookupType type, bool err)
{
    ...
    if (err) {
		...
		exit(1); // If you want to call lookupEntry(sth, sth, false), remove this line
	}
    return NULL;
}
...
{
	if((type1->kind == TYPE_ARRAY) && (type1->refType->kind == type2->kind)) {
		if(type1->size == 0)
			return true;
		else if(type1->size == 1)
			return true;
	}
	if((type2->kind == TYPE_ARRAY) && (type2->refType->kind == type1->kind)) {
		if(type2->size == 0)
			return true;
...
SymbolEntry * getParameter (SymbolEntry * e, int num) {
	if(e == NULL) {
		printf("Cannot get parameter of NULL function.\n");
		return NULL;
	}

	SymbolEntry * temp = e->u.eFunction.firstArgument;
...
void printParameters (SymbolEntry * e)
{
	if(e == NULL) {
		printf("Tried to print parameters of NULL function.\n");
		return;
	}
...
void printParam (SymbolEntry * e)
{
	if(e == NULL) {
		printf("Tried to print a NULL parameter.\n");
		return;
	}
